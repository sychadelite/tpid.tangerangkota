      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex flex-wrap">
                  <h3 class="card-title">Tabel data profil</h3>
                ...
                <div>
                  <button type="button" id="btn-modal-add-profil" class="btn btn-primary" data-toggle="modal" data-target="#addProfilModal">
            ...
            <div class="card-body">
              <table id="table_profil" class="table table-hover table-striped">
                <thead></thead>
                <tbody></tbody>
                <tfoot></tfoot>
              </table>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addProfilModalLabel">Tambah Data Profil</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form id="formAddProfil" class="needs-validation" enctype="multipart/form-data" novalidate>
            <input type="hidden" name="<?= $content['csrf']['name']; ?>" value="<?= $content['csrf']['hash']; ?>" />
            <input type="hidden" id="category" name="category" value="profil" />
            <div class="form-row">
              <div class="col-md-12 mb-3">
                <label for="title">Title Profil</label>
                <input type="text" class="form-control" id="title" name="title" value="<?= set_value('title'); ?>" placeholder="Title Profil .." required>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" form="formAddProfil">Save changes</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="editProfilModal" tabindex="-1" aria-labelledby="editProfilModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="formEditProfil" class="needs-validation" enctype="multipart/form-data" novalidate>
            <input type="hidden" name="<?= $content['csrf']['name']; ?>" value="<?= $content['csrf']['hash']; ?>" />
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="submit" class="btn btn-primary" form="formEditProfil">Save changes</button>
        </div>
      </div>
    </div>
  </div>

  <div class="modal fade" id="deleteProfilModal" tabindex="-1" aria-labelledby="deleteProfilModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="deleteProfilModalLabel">Hapus Data Profil</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form id="formDeleteProfil" class="needs-validation" novalidate>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-danger" form="formDeleteProfil">Hapus</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
  ...
  $(function() {
    ...
    var previewAddNode = document.querySelector("#addProfilModal #template-add-file");
    previewAddNode.id = "";
    previewAddTemplate = previewAddNode.parentNode.innerHTML;
    previewAddNode.parentNode.removeChild(previewAddNode);

    var previewEditNode = document.querySelector("#editProfilModal #template-edit-file");
    previewEditNode.id = "";
  ...
  $(window).on('resize', function() {
    table_profil.columns.adjust(); // Adjust the column widths on window resize
  });

  $(document).on('click', '[id^="btn-modal-add-profil"]', function() {
    ...
    const addConfig = {
      ...
      clickable: "#addProfilModal .fileinput-button",
      uploadMultiple: true,
      maxFiles: 10,
      maxFilesize: 2,
  ...
  $(document).on('click', '[id^="btn-modal-edit-profil"]', function() {
    let row_id = $(this).data('profil');
    // Perform AJAX call
    $.ajax({
      ...
      success: function(response) {
        ...
        $(`#editProfilModal #id`).val(response.data.id);
        $(`#editProfilModal #title`).val(response.data.title);
        $(`#editProfilModal #slug`).val(response.data.slug);
        $(`#editProfilModal #category_id`).val(response.data.category_id);
        $(`#editProfilModal #status`).bootstrapSwitch('state', response.data.status === 'active', true);
        ...
        const editConfig = {
          ...
          clickable: "#editProfilModal .fileinput-button",
          uploadMultiple: true,
          maxFiles: 10,
          maxFilesize: 2,
          accept: function(file, done) {
  ...
  $(document).on('click', '[id^="btn-modal-delete-profil"]', function() {
    let row_id = $(this).data('profil');

    $(`#deleteProfilModal #id`).val(row_id);

    $('#deleteProfilModal').modal('show');
  });

  $('#formAddProfil').submit(function(e) {
      ...
      error: function(xhr, status, error) {
        if (xhr.status === 401) {
          ...
          $.each(errors, function(field, message) {
            var inputField = $(`#formAddProfil [name="${field}"]`);
    ...
    $.ajax({
      ...
      error: function(xhr, status, error) {
        if (xhr.status === 401) {
          ...
          var errors = JSON.parse(xhr.responseText).data;
          $.each(errors, function(field, message) {
            var inputField = $(`#formEditProfil [name="${field}"]`);
            var invalidFeedback = inputField.next('.invalid-feedback');
  ...
  $('#formDeleteProfil').submit(function(e) {
    // Prevent form submission
    e.preventDefault();
  ...
  $(function() {
    table_profil = $('#table_profil').DataTable({
      "paging": true,
      "lengthChange": true,
      "searching": true,
      "ordering": true,
        ...
        },
        {
          ...
          render: function(data, type, row, meta) {
            ...
            <div class="d-flex flex-nowrap align-items-center justify-content-end" style="gap: 1rem;">
              <button id="btn-modal-edit-profil-${row.id}" data-profil=${row.id} type="button" class="btn btn-sm btn-info d-flex flex-nowrap align-items-center justify-content-center">
                <i class="fas fa-edit mr-1"></i>
                Edit
              </button>
              <button id="btn-modal-delete-profil-${row.id}" data-profil=${row.id} type="button" class="btn btn-sm btn-danger d-flex flex-nowrap align-items-center justify-content-center">
