  ...
  public function get_all_by_category($category)
  {
    ...
    if ($authorizationHeader) {
      ...
      if ($this->UserGroupModel->get_data('id', $check_user->user_group_id)) {
        ...
        $response = [
          "status" => true,
          "message" => 'Profile by category retrieved',
          "data" => $profile,
        ];
        http_response_code(200);
        echo json_encode($response);
      ...
      if ($this->UserGroupModel->get_data('id', $check_user->user_group_id)) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $payload = [
            ...
            "enable_service" => $this->input->post('position') ?? 0
          ];

          $check_category = $this->CategoryModel->get_data('id', $payload['category_id']);
          if ($check_category) {
            $insert_profil = $this->ProfileModel->insert_data($payload);
            if ($insert_profil) {
              if (isset($_FILES['files'])) {
                $files = $this->processFiles($_FILES['files']);
                if ($files['status']) {
                  $last_profil_id = $this->db->insert_id();
                  $processedFiles = array();
                  for ($i = 0; $i < count($files['data']); $i++) {
                    if ($files['data'][$i]['status']) {
                      $this->ProfileFilesModel->insert_data([
                        "profile_id" => $last_profil_id,
                  ...
                  if (count($processedFiles) > 0) {
                    $where = "id=" . $last_profil_id;
                    $this->ProfileModel->update_data($where, ["cover" => $processedFiles[0]]);
                  }
                }
              }
    ...
    if ($authorizationHeader) {
      ...
      if ($this->UserGroupModel->get_data('id', $check_user->user_group_id)) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $is_unique_profil = $this->ProfileModel->get_unique_data($this->input->post('id'), $payload['title'], $payload['slug']);
          if ($is_unique_profil) {
            $error_data = [];
            foreach ($is_unique_profil as $key => $value) {
              $error_data['title'] = $value->title === $payload['title'] ? 'Profil Title telah digunakan' : null;
              $error_data['slug'] = $value->slug === $payload['slug'] ? 'Profil Slug telah digunakan' : null;
            }
            $response = [
              "status" => false,
              "message" => 'Payload is not satisfied',
            ...
            $check_category = $this->CategoryModel->get_data('id', $payload['category_id']);
            if ($check_category) {
              $current_profil_id = $this->input->post('id');
              $where = 'id=' . $current_profil_id;
              $update_profil = $this->ProfileModel->update_data($where, $payload);
              if (isset($_FILES['files'])) {
                // File name does exist on server and DB but doesn't exist on payload
                $file_to_delete = $this->ProfileFilesModel->get_all_data('profile_id', $current_profil_id, null, [], 'file_name', $_FILES['files']['name']);
                $this->removeFiles($current_profil_id, $file_to_delete);

                // File name already exists on server and DB
                $file_available = $this->ProfileFilesModel->get_all_data('profile_id', $current_profil_id, 'file_name', $_FILES['files']['name']);
                ...
                $file_to_delete = $this->ProfileFilesModel->get_all_data('profile_id', $current_profil_id);
                $this->removeFiles($current_profil_id, $file_to_delete);
              }
  ...
  public function destroy()
  {
    ...
    if ($authorizationHeader) {
      ...
      if ($this->UserGroupModel->get_data('id', $check_user->user_group_id)) {
        ...
        $delete_category = $this->ProfileModel->delete_data($where);
    ...
    if ($files) {
      for ($i = 0; $i < count($files); $i++) {
        if (!empty($files[$i]->file_path)) {
          ...
          $this->ProfileFilesModel->delete_data($where_delete, $id);
        }
      }
    }
    return true;
