...
<script>
  ...
  window.onload = function() {
    document.querySelectorAll(".form-block-input").forEach((formBlockInput) => {
      new mdb.Input(formBlockInput).init();
    });
  };
  ...
  Array.prototype.slice.call(forms).forEach((form) => {
    ...
    form.addEventListener('submit', (event) => {
      ...
      const btnDefaultText = btn.innerText
      ...
      const timeout = setTimeout(async () => {
        ...
        btn.innerHTML = await btnDefaultText || `Try Again`;
    ...
    function validityEveryInput() {
      ...
      inputs.forEach(input => {
        ...
        if (error) {
          ...
          // input.closest('[class^="form-"]')?.parentElement.classList.add('mb-3'); // removed
        }
      });
    }
